...
public class KullaniciPostgreSQLSurucusu implements IKullaniciVeriTabaniServisi {
    ...
    public boolean kullaniciDogrulama(String kullaniciAdi, String parola) {
        try {
            Connection conn = DriverManager.getConnection("jdbc:postgresql://localhost:5432/can", "postgres", "12345");
            if (conn != null) {
                ...
                if (_kullaniciAdi.equals(kullaniciAdi) && _parola.equals(parola)) {
                    System.out.println("*******Kullanıcı Girişi Veritabanına bağlandı!***********");
                    return true;
                } else {
                    System.out.println("********Hatalı Giriş Bağlantı Başarısız!************");
                    return false;
                }
            } else {
